// 시트 CSV 파싱 + '총 누적 지표' 추출 — 순수 함수(네트워크·DB 의존 0, 단위 테스트 대상)
import { SHEET_LABEL_TO_SLUG, type SyncTargetSlug } from "./mapping";
...
// 정규화 라벨 → slug 역인덱스
const LABEL_LOOKUP: Record<string, SyncTargetSlug> = Object.fromEntries(
  Object.entries(SHEET_LABEL_TO_SLUG).map(([label, slug]) => [
...
// 그리드에서 '총 누적 지표' 라벨 ↔ 다음 행 숫자 추출. 라벨 셀 발견 → 같은 열의 다음 행 값(비면 우측 3칸 스캔, 병합셀 오프셋 대비). slug 당 첫 매치만. 숫자 없으면 제외(기존 값 보존).
export function extractCumulativeMetrics(grid: string[][]): ParsedMetric[] {
  ...
  for (let r = 0; r < grid.length - 1; r++) {
    ...
    for (let c = 0; c < labelRow.length; c++) {
      const slug = LABEL_LOOKUP[normalizeLabel(labelRow[c] ?? "")];
