...
> **Figma가 SSOT**. 의도서/기획안에서 정의된 7개 페이지 중 현재 시안이 있는 것은 2개뿐. 나머지 5개는 "없는 것으로 추정"(사회공헌국 답변). 통합되었을 가능성도 있어 *추가 Figma 확인 필요*.

| 의도서 페이지 | 라우트(잠정) | Figma 시안 | Figma 노드 | 비고 |
|---|---|---|---|---|
| 메인 홈 (1920px) | `/` | ✅ 있음 | `126:11815` (1920×3803) | 큰 데스크톱 폭 |
| 메인 홈 (1440px) | `/` | ✅ 있음 | `96:7689` (1440×3803) | 1440 폭 변형. 1920과 컴포지션 일부 다름 |
| 소식 (=임팩트 스토리) | `/stories` | ✅ 있음 (목록+상세) | TBD (사용자 추가 공유 예정) | 의도서는 "임팩트 스토리"라 부르지만 Figma는 "소식" |
| 소개 | `/about` | ❌ 없음 | — | 메인 안에 통합 가능성 (확인 필요) |
| 투명성 리포트 | `/reports` | ❌ 없음 | — | 의도서 §5.3 — 인포그래픽 대시보드 |
| 권역별 현황 | `/regions` | ❌ 없음 | — | |
| 파트너 스토리 | `/partners` | ❌ 없음 | — | 의도서 §5.4 |
| 참여하기 | `/join` | ❌ 없음 | — | 헤더 CTA |
