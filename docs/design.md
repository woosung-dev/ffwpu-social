...
> ✅ **현재 추출 상태 (2026-05-30)**: 1440 폭 깊이 추출 + **1025~1439 / 768~1024 / 375~767 작은 사이즈 시안도 수령 완료**(스크린샷 기반 시각 메모). 각 BP 범위마다 디자이너가 _양 끝 두 폭_(예: 1025/1439)을 따로 그려준 의도는 BP 사이 보간 가이드.
>
...
> - 소식 페이지 상단 **Banner 띠 정식 등장** (D-4 에 _사용자 지적으로 제거_ 했던 위치) — 카피 "Sow Good 가족이 아니어도, 같은 동네가 아니어도, 밥상을 함께하는 사람이 있다면 우리는 이미 식구입니다." (사용자 결정 2026-05-30 "신 Figma 정식 도입").
> - 부모 노드 3종(`97:10250` / `96:5908` / `95:9359`) · 자산 매핑(KPI 사진·파트너 로고 등) ID **변동 없음**.
> - 사회공헌국 원본 사진 11장 + 파트너 BI 5장 _사용자 조달_ 대기 (`docs/TODO.md` 신규 항목).
...
> **Figma가 SSOT** (ADR-000). 의도서 §5의 7 페이지 IA는 **ADR-006으로 폐기**. Figma 4 헤더 메뉴 기준으로 재정렬. 메뉴 3개는 _랜딩 섹션 앵커_, 1개는 별도 페이지(소식).
...
| 쌀나눔 프로젝트 (메뉴 4) | `/#story` 앵커 _추론_ | ✅ 랜딩 StorySection _추정_ | `96:7834`              | 사회공헌국 확인 필요                                          |
...
| **SearchInput** (구현완료) | 목록 툴바 (Figma 외) | "더 많은 소식" 행에 탭(좌)+검색(우, md↑ 한 줄/375 세로). 제목+태그 ILIKE. **Figma 미존재 — familyfed 1323-8705 패턴 + 우리 토큰, ADR-036** |
...
두 시안 모두 _동일한 6개 직속 자식 컨테이너_ — 같은 페이지의 두 폭 디자인임을 구조로 확인.
...
> ✅ **2026-05-31 1920 자식 섹션 ID 재추출 완료** (`get_metadata({nodeId: "331:7984"})` 1회). 옛 1920 자식 ID(`126:11816/11892/11954/11986/12005/12076`) 는 신 메타에서 전부 사라짐 — _완전 폐기_. 1440 컬럼은 변동 없음이라 그대로 신뢰.
...
| 5   | PartnersSection | `332:9444`   | 547  | **6 슬롯 2×3 그리드** — 단 `image 38` 이 슬롯 #3·#5 _중복 표시_ (디자이너 placeholder 채우기). **운영 5장 유지** |
...
> ✅ **`wide:` breakpoint 버그 — 수정됨** (2026-06-06). 원래 `wide:` 가 base 만 override 하고 md/lg 는 못 이겼음(Tailwind v4 ^4.0.0).
> **근본 원인 = 단위 불일치**: 기본 breakpoint(sm~2xl)는 `rem` 인데 `--breakpoint-wide` 만 `1440px` 라, px breakpoint 가 rem 그룹과 분리돼 생성 CSS 에서 *앞에* 정렬 → base 만 이김. (격리 테스트: 800→md22 / 1200→lg33 / 1920→wide무시33. `.next` 클린 후도 동일이라 stale 아니었음. arbitrary `min-[1440px]:` 도 동일.)
> **수정**: `globals.css` `--breakpoint-wide: 1440px → 90rem`(=1440px, 기본값과 단위 통일). 이후 1440·1920 에서 `wide:max-w-[1200px]` 정상 적용 확인. **이제 전 코드 `wide:` 가 md/lg 를 정상 override.**
> **교훈**: 커스텀 breakpoint 는 **반드시 기본값과 같은 단위(`rem`)** 로 정의할 것. px 로 두면 정렬이 깨진다.

### 섹션 컨테이너 폭 (SectionContainer) — 구간별 고정폭

> Figma 4 프레임 실측(전 섹션 동일, Contents x좌표 역산, 768·1024·1439·1920 검증 2026-06-06): **mobile 375~767 유동**(343→729·gutter ~16) / **tablet 768~1024 고정 648** / **desktop 1025~1439 고정 905** / **wide 1440~1920 고정 1200**. gutter 만 커지는 패턴 — "겉만 줄어든다".
> 구현: `mx-auto w-full px-4 md:max-w-[648px] md:px-0 lg:max-w-[905px] wide:max-w-[1200px]`. 각 구간 뷰포트 > max-w 라 **max-w 가 곧 고정폭**, gutter 는 mx-auto 중앙정렬 여백. (Figma `max-width:1680` 은 프레임 속성일 뿐 실제 콘텐츠는 1200.) 미세: lg=1024 라 정확히 1024px 는 905(Figma tablet 648) — 1px 경계, 필요시 `min-[1025px]:`.
...
| 767~375   | `135:11268` (375) + `135:12490` (767) | _양 끝 모두 그림_                   |

### 공지사항(notices) BP별 노드 (2026-07-08 추가)

> 게시판형 공지 목록/상세. 디자이너가 구간별 프레임 10 제공. 실측·조치 상세는 `docs/design/notices-fidelity-2026-07-08.md`, ground truth PNG `docs/design/figma-export/notices/notices-{list,detail}-{375,767,768,1025,1440}.png`.
> **헤더·푸터·배너(SubBanner)는 프레임에 포함돼 있어도 참고 대상 제외** (사용자 지시) — 중앙 콘텐츠만 정합.

| 범위 | 목록 노드 | 상세 노드 |
| --- | --- | --- |
| 1440 (wide) | `1103-7882` | `1104-10813` |
| 1025 (lg) | `1149-7972` | `1149-11851`(부모=프레임) |
| 768 (md, 고정 648) | `1149-8742` | `1149-11987` |
| 767 (모바일 유동 735) | `1149-9786` | `1149-12667` |
| 375 (모바일) | `1149-9301` | `1149-12395` |

- 767·768 두 프레임 = SectionContainer 768 경계 시연(767 유동 px16 / 768 고정 648). SectionContainer 밴드 그대로 사용.
- **목록**: 모바일(<768)은 No·Date 열 제거·Title 단일. 행 높이 40·48(375)→44·56·62(768)→53·62·70(1025·1440). eyebrow18/h1 32 전 BP 고정.
- **상세**: `타이틀(중앙)→본문→다운로드(본문 아래)→구분선→이전/다음`. h1 28(375)→32(md↑), 블록 gap 60, 다운로드→구분선 70.

### 반응형 컴포지션 변화 매트릭스

각 BP에서 _섹션 컴포지션이 어떻게 바뀌는지_ 핵심만.
...
>
> - **컴포지션 경계는 1024(`lg`) 단일.** Figma 1024 시안(`332:9254`)은 _데스크탑 컴포지션_(KPI 벤토·Story 좌우 2단·ArticleGrid 사이드 헤더·Partners 가로)에 속함. 진짜 태블릿 전용 밴드는 **768~1023 하나뿐**. "1024 = 세로/2x2" 표기는 오류.
...
- _깨지면 안 되는 요소_: 헤더 메뉴 칩(SUIT ExtraBold 14/16px), 카드 라운드, 보라/오렌지 컬러.
...
- **로고**: BI PPT의 디자인 A/B 검토 결과가 **현재 Figma 디자인에 그대로 반영**됨. Figma의 Sow Good 워드마크·해바라기·꽃 일러스트가 _최종 BI 자산_ (확정 2026-05-26)
...
- **모바일(375~767)**: 로고 + 선택 메뉴 1개만 표시 + 검색 (다른 메뉴는 드로어/햄버거 예상 — _현재 시안에 없음, 확인 필요_)
...
- **상태**:
  ...
  - `None`: 이미지 없는 _플레이스홀더_ — 보라 그라디언트 배경 + "보도자료" 텍스트 over
...
- ArticleCard: hover 시 좋아요 표시. _클릭 시 상세 페이지 이동_ 가정.
- Heart: 클릭 시 +1 카운트. _익명 좋아요(로그인 없음)_ 가정 — 백엔드 모델 확인 필요.

### 누락·미확인

- **검색 결과**: 검색 아이콘은 있으나 검색 UI 별도 시안 없음
- **모바일 햄버거 드로어**: 헤더 모바일(375~767)에 메뉴 1개만 노출 — 나머지 메뉴 어떻게 접근하는지 미확인
- **폼**: 의도서가 정의한 파트너십·참여 폼 — _Figma에 없음_. 사이트에 폼이 있어야 하는지 확인 필요
...
### 2) KpiSection (`96:7773`, 1440×952) — "숫자로 보는 우리의 변화"

- **좌측**: 헤딩 "숫자로 보는 / 우리의 변화" (SUIT Bold 36px, `#242424`) + 설명 (SUIT Medium 16px)
  - ⚠️ **카피는 Figma 시안 기준.** 구현 카피는 2026-07-30 사용자 요청으로 "숫자로 보는 / 참사랑 실천" + 새 설명문으로 교체됨 (ADR-038 개정). 레이아웃·타이포는 그대로.
...
- **우측**:
  ...
  - 헤딩 "밥이 사랑입니다 / 나누는 우리는 식구입니다" — SUIT Bold 32px, `#242424`, _우측 정렬_
...
- **좌측 다크 블록** (319px, `#242424` 배경, 라운드 12px):
  ...
  - 버튼 "더 많은 스토리 보러가기" + 화살표 (텍스트만, 배경 없음)
- **우측 마조네리 3열 그리드** — StoryCard(`44:1840`) 인스턴스 6개:
  - 다양한 높이: 256 / 425 / 425 / 337 / 278 / 381 — _마조네리(Masonry) 레이아웃_
...
> ⚡ **의도서 §5.2의 3 카테고리(가족치유/지역봉사/환경캠페인)가 살아있음**. ADR-006으로 의도서 IA 폐기했어도 _카테고리 분류 체계는 유산이 살아있다_.
...
→ 카드 안 카테고리 태그(`#B35FEB` 텍스트)도 위 5개 enum 중 하나. 시안에서 본 "지역봉사"·"보도자료"·"쌀나눔" 라벨은 _카드 상태별 다른 콘텐츠_ 또는 _카테고리 enum 값_.
→ ArticleCard None 상태의 "보도자료" 라벨은 그라디언트 placeholder 표시 — 별도 카테고리가 아닌 _이미지 없음_ 시각 표시.
...
> ⚡ **새 폰트 발견 — Pretendard**. SUIT 외에 페이지네이션 번호에 **Pretendard Medium/Regular** 사용. 두 폰트 시스템 혼용. _디자인 토큰 통합 시 의도 확인 필요_ (의도된 혼용인지, 시안 정리 미흡인지).
...
6. **"더 많은 소식 살펴보기"** (`93:8865`):
   - 헤딩 SUIT Bold 20px
   - 관련 글 3개 카드 — 컴포넌트 ID `464:3046/3043/3044` (메인 ArticleCard와 _다른 컴포넌트_)
...
카테고리 칩과 별개의 _해시태그_ 컴포넌트. 본문 게시글 분류.
...
| 사이트                                              | 배울 점                                         | 우리와 다른 점                                        |
| --------------------------------------------------- | ----------------------------------------------- | ----------------------------------------------------- |
| [지구랩 ZIGULAB](https://zigulab.codextyle.com/)    | 차분한 톤, 넉넉한 여백, 아티클+보고서 병렬 구조 | 우리는 _사람 이야기_ 중심성이 더 강해야 함            |
...
> 사회공헌국 1인 운영자(50대·IT 비숙련) 대상 CMS. 사용자 사이트와 _분리된 디자인 시스템_ — 톤은 공통(보라 brand-primary + warm + kpi 4색), 위계·인터랙션 규약은 어드민 전용.
