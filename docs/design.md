...
> ✅ **현재 추출 상태 (2026-05-26)**: 1440 폭 깊이 추출 + **1025~1439 / 768~1024 / 375~767 작은 사이즈 시안도 수령 완료**(스크린샷 기반 시각 메모). 각 BP 범위마다 디자이너가 *양 끝 두 폭*(예: 1025/1439)을 따로 그려준 의도는 BP 사이 보간 가이드.
...
|---|---|---|---|---|
| 메인 홈 (1920px) | `/` | ✅ 있음 | `126:11815` (1920×3803) | 큰 데스크톱 폭 |
...
| 3 | Banner | `125:8915` | 1440×132 | 안내 배너 ("Sow Good — 따뜻한 진심을 담아 / 나누는 진실의 활동들을 소개합니다") |
...
| 후보 | 위치 (페이지) | 메모 |
|---|---|---|
| **Banner** | 목록·상세 공통 | "Sow Good — 따뜻한 진심…" 가로 안내 배너. 재사용 강함 |
...
|---|---|---|---|---|---|
| 1 | **HeroBanner** | `96:7690` | `126:11816` | 740 | Header(=instance) + 슬로건 + Sow Good 해바라기 일러스트 |
| 2 | **KpiSection** | `96:7773` | `126:11892` | 952 | KPI 카드(45,217명+ 등 4개) + Sow Good 노란 카드 |
| 3 | **StorySection** | `96:7834` | `126:11954` | 573 | 인물·쌀 패키지 미니 그리드 + "쌀 사람들의 / 나누는…" 카피 |
| 4 | **ArticleGrid** | `96:7877` | `126:11986` | 982 | ArticleCard 그리드 (size 2/3 추정) — "고소 사랑의 향기가" 외 6개 |
| 5 | **Section5** | `96:7897` | `126:12005` | 457 | ? — 추가 CTA 또는 후원·파트너 섹션 추정 (추출로 확인) |
| 6 | **Footer** | `126:10897` | `126:12076` | 99 | 후원사 로고 라인 + 카피라이트 |
...
| | 1920px (`126:11815`) | 1440px (`96:7689`) |
|---|---|---|
| 스크린샷 | `landing-126-11815-1920px.png` | `landing-96-7689-1440px.png` |
| 전체 크기 | 1920×3803 | 1440×3803 |
| 컴포지션 | 동일 — 6개 섹션 같은 순서 | 동일 |
| 차이 | 카드 그리드가 더 넓게 펼침, 여백 큼 | 더 압축, 카드 간 여백 좁음 |

![1920](./design/screenshots/landing-126-11815-1920px.png)
...
| 범위 | 분류 | 작은쪽 노드 | 큰쪽 노드 |
|---|---|---|---|
| 1920~1440 | Large Desktop | `96:7689` (1440) | `126:11815` (1920) |
| 1439~1025 | Desktop | `97:8573` (1025) | `126:11398` (1439) |
| 1024~768  | Tablet | `97:9014` (768) | `126:10980` (1024) |
| 767~375   | Mobile | `99:6950` (375) | `126:12232` (767) |
...
|---|---|---|---|---|
| **Header** | 4 메뉴 노출 + 검색 | 4 메뉴 노출 + 검색 | **햄버거** + 단일 메뉴 칩 + 검색 | **햄버거** + 단일 메뉴 칩 + 검색 |
| **HeroBanner** | 가로 2단 (슬로건 / 해바라기) | 가로 2단 (좁아짐) | 세로 스택? (해바라기 위·슬로건 아래) | 세로 스택, 해바라기 작아짐 |
| **KpiSection** | 4 카드 가로 + 부가 | 4 카드 가로 (압축) | 2x2 마조네리 | **2 카드 세로 마조네리** |
...
| **Section5 Partners** | 5 로고 가로 | 5 로고 가로 | 3+2 줄? | 작게 스택 |
...
1. **헤더 햄버거 전환**: 1025 이상 = 4 메뉴 노출, 1024 이하 = 햄버거 + 단일 활성 메뉴 칩 노출
2. **그리드 열 수**: 마조네리 3 → 2 → 1, KPI 4 → 2x2 → 2 → 2 세로
