...
>
> 🔴 **2026-06-05 픽셀 정합 재감사 — 과거 "정합 완료" 표기 정정**: 본 문서·plans 의 "Figma 정합 검증 완료" 는 **실제 렌더 기준 부정확**했다. live 렌더 vs Figma 재검증 결과 — (1) **stale dev 서버**로 `_next/image`(MinIO) 가 400 차단되어 카드 이미지가 빈 플레이스홀더로 렌더(코드 아님; `pnpm dev` 재시작으로 해소), (2) 카드·마조네리 **열 전환이 Figma 아트보드보다 1단계 일찍** 발생(소식목록 768=3열→2열, 랜딩 ArticleGrid 768=3열→2열, 상세 관련글), (3) Hero dead 폰트 참조 등 **5건 교정**. 상세·근거: `docs/design/audit-2026-06-05.md`. **교훈: docs 의 "검증 완료" 를 신뢰하지 말고 live vs Figma 로 재확인.**
...
- **최근 확인 날짜**: 2026-06-05 (**픽셀 정합 재감사 — live vs Figma, `audit-2026-06-05.md`**) / 2026-06-03 (반응형 매트릭스 정정 + **KPI·Hero 2차 재정정** — 아래 §반응형 컴포지션 매트릭스 정정 노트) / 2026-05-30 / 2026-05-26
