...
- [x] S9 **상위 고정(순서 드래그)** — `pinned_rank`(마이그레이션 0014) + partial unique index + 공개 고정우선 정렬(번호 대신 '고정' 칩) + 어드민 상단 `NoticePinOrderManager`(HeroOrderManager 이식) + 2-phase 저장·advisory lock 740033 + 발행 해제 시 동반 해제 + ADR-043. tsc0·lint0·**test103**·build✓ + E2E PASS(picker 발행만·키보드 재정렬·DB 순서 영속·공개 반영·고정 배지·해제 시 자동 해제). **시각 시안(design-shotgun)은 후속.**
...
- 2026-07-08 (S9 상위 고정): "관리자 페이지 전혀 안 됨" 보고 조사 → 코드는 완성, 원인은 **migrate 0013 미적용**(테이블 부재 500). 별개로 "상위 고정" 신규 요청(순서 드래그·목록 화면 조작). news `hero_rank` 패턴 이식으로 구현. **공개 번호(No.) 공식 무변경** — 고정 우선 정렬 시 고정 글이 전역 상단 P개를 점유하고 번호를 숨기므로 비고정 행이 자연히 `(total−P)…1` 로 이어짐(검증 4·3·2·1). 어드민 조작은 목록 상단 카드 1곳(작성/수정 폼 무변경, 사용자 지시). 최대 3개. 발행만 고정(예약·임시 picker 제외). 발행 해제 시 `clearPinnedRank` 동반. E2E 로컬 검증 후 시드 원복(고정 0).
- 잔여: prod 배포 시 `pnpm db:migrate`(0013·0014) 필요. **상위 고정 시각 시안(design-shotgun) 후속** — 공개 목록 고정 표시(칩·배경 구분)는 Figma 시안 부재로 1차 최소 마커.
