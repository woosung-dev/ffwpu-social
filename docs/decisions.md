...
- **`deviceSizes` 축소 / `sizes` 정밀화** — TTL 수정 후엔 조합당 주 1회만 청구되어 실익이 미미한 반면 4-BP 렌더 재검증 비용이 든다. 별도 작업으로 분리 → `deviceSizes` 축소는 ADR-050 에서 채택(대역폭 회귀 0 검증 후).
...
## ADR-050: 이미지 옵티마이저 원격 호스트 고정 + srcset 폭 7단 + 설정 회귀 펜스

- **Status**: Accepted
- **Date**: 2026-08-06
- **관련**: ADR-049 (캐시 TTL) — 같은 `next.config.ts images` 블록을 다루지만 **목적이 다르다.** 049 는 *반복 재변환* 제거, 050 은 *남용 차단 + 변환 상한 축소*.
...
502/404 는 그 버킷이 없어서 난 것이고, **실존하는 제3자 객체를 가리키면 200 + 변환 과금**이다. 나머지 둘도 문제였다 — `*.s3.amazonaws.com` 은 아예 미사용이고, `*.r2.cloudflarestorage.com` 은 코드 주석이 스스로 "인증 API 엔드포인트라 공개 렌더용 아님"이라 적어놓고 화이트리스트에 있었다.

### Decision

`next.config.ts` 단일 파일. **컴포넌트 코드는 손대지 않는다.**

1. **`remotePatterns` 를 `NEXT_PUBLIC_S3_PUBLIC_URL` 파생 단일 패턴으로 교체.** 와일드카드 3종을 제거한다. 프로토콜·호스트·포트를 URL 에서 파생하므로 **로컬 MinIO(`http://localhost:9000`)와 prod R2(`https://pub-*.r2.dev`)를 dev 분기 없이 같은 코드로 덮는다.** `src/lib/s3.ts getPublicUrl()` 과 같은 값에서 나오므로 드리프트가 불가능하다. 프로덕션 빌드에서 패턴 생성에 실패하면 **`throw` 로 빌드를 실패시킨다** — 조용한 이미지 전멸보다 빌드 실패가 낫다.
2. **`deviceSizes` 8→6 · `imageSizes` 7→1** (허용 폭 15→7). 원본이 클라 리사이즈로 **긴 변 2560px 상한**(`storage/image-resize.ts`)이라 `3840` 은 업스케일 금지(`withoutEnlargement`)로 2560 을 돌려준다 — 화질 이득 0, 변환만 소비. `2048` 은 어떤 실제 슬롯도 요구하지 않고, `256` 은 DPR1 태블릿 전용 구간이라 실사용이 없다. **`750`·`1080` 은 유지한다** — 국내 주력 폰(375@2=750, 360@3=1080)이 정확히 착지하는 폭이라 지우면 한 칸 위로 점프해 첫 방문 바이트가 늘어난다. `deviceSizes[0]=640` 도 고정 — Next `getWidths()` 가 후보 임계값을 `deviceSizes[0] × min(vw%)` 로 계산해 이 값을 바꾸면 3개 컴포넌트의 후보 집합이 통째로 흔들린다.
...
- **제3자가 우리 변환 쿼터를 소비하는 경로가 닫힌다.** ADR-049 가 낭비를 줄였다면 050 은 *외부 유입*을 막는다 — 둘은 대체재가 아니다.
- **대역폭 회귀 0.** Next `getImgProps` 실제 코드 경로로 검증한 결과 3개 컴포넌트 모두 **추가된 폭이 0개**다(제거만 발생): MediaCard 10→7 · FeaturedStoryCard 9→7 · ArticleCard 8→6. 어떤 기기도 갑자기 더 큰 이미지를 받지 않는다.
- 관측 가능한 유일한 차이는 DevTools Network 탭이다 — iPad 가로(1024@2)가 `w=2048` 대신 `w=1920` 을, 5K 디스플레이가 `w=3840` 대신 `w=1920` 을 요청한다. 둘 다 실제 슬롯 대비 각각 3.3배·1.65배 오버샘플이라 육안 차이가 없고, `w=3840` 은 원래도 2560px 를 받고 있었다.
- **커스텀 도메인 연결 시 `NEXT_PUBLIC_S3_PUBLIC_URL` 만 바꾸면 된다** — remotePatterns 를 따로 손댈 필요가 없어졌다. 반대로 그 환경변수를 잘못 넣으면 전 커버가 400 이 되므로 `docs/deploy-env-checklist.md` 에 반영했다.
- 폭 화이트리스트 강제는 서버 `image-optimizer` 의 `sizes.includes(width)` 에서 일어나 **배포 후 curl 로만 최종 확인 가능**하다 — 배포 후 체크리스트로 이관.
- **범위 밖으로 확인된 별건** — `cacheComponents: true` 가 켜져 있는데 `"use cache"` 사용처가 0이다(`docs/tech.md` §캐싱 전략에 설계만 존재). `revalidateTag` 도 0건이라 `revalidatePath` 가 데이터 신선도에 기여하는 바가 없다. 이미지 쿼터와 무관하며 **Fluid Active CPU(37%) · Edge Requests(28%)** 를 밀어올리는 요인이다. 무효화 설계를 동반하므로 별도 스프린트로 분리한다.
