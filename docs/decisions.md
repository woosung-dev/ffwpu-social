...
**개정(2026-07-30, 사용자 요청 — 카피만):** `#kpi` 메뉴·섹션 명칭을 **"숫자로 보는 우리의 변화" → "숫자로 보는 참사랑 실천"** 으로 변경. KpiSection 헤딩·설명 카피도 함께 교체(설명: "가정연합은 대한민국 현대사와 동고동락하며 … 따뜻하고 선한 공동체로 함께 하고 있습니다."). 어드민 사이드바 그룹명·`/admin/kpi` 제목·설명도 `ADMIN_COPY` SSoT 에서 동일 명칭으로 정합. 매핑·라우팅·스키마 변경 0 (`section: "kpi"` id 불변).
...
- **'고정' 칩 제거 → 번호 유지 + 선행 핀 아이콘 + 보라 텍스트.** Figma 고정 행 = 번호(`#c8a3e6` 18px) + `No/PinIcon`(fill `#e1c8f9`, 24px) + 제목(`#a34df3` 18px) + 날짜(`#c8a3e6`) + **flat 배경 `#fcfaff`**(시안 B 의 진한 `#efe4ff` 아님). 구분선(`#d9c2f5`)만 유지.
...
**개정(2026-07-09, 사용자 요청 — 비-Figma 추가분 감사 정리):** 사용자가 하단 그라데이션을 "Figma에 없는데 반영됐다"고 지적 → 전수 감사. 결과 **그라데이션(`1103:7941 Background` = `#F9F4FF/80→white` h590)·스크롤탑(`1103:8079`)은 실제 Figma에 존재**(변경 없음). 진짜 비-Figma 추가분 2건 정리:
- **일반 행 zebra(`#f9f9fc`/white) 제거 → 투명.** Figma 일반 행(`1104:9979`)은 배경 fill 없음. 이전 정합 문서가 "지브라 기존 일치"로 오기재한 것을 정정.
...
4. **억제 상태는 브라우저에만 저장** — `sg_popup_dismissed`(localStorage, `{[id]: 만료 epoch}`, 7일)만. 서버 전송 없음 — 익명 하트(ADR-026)와 동일 계층. **그냥 닫기는 아무것도 저장하지 않는다** — 새로고침·랜딩 재진입 시 다시 노출 (사용자 결정 2026-07-18: sessionStorage 는 새로고침에도 유지돼 "닫으면 새로고침까지 안 뜨는" 기대 불일치 → 세션 억제안 폐기). 노출이 랜딩 한정이라 재노출 피로 리스크는 낮고, 억제는 "일주일간 보지 않기" 단일 수단으로 일원화.
5. **UI 는 shadcn Dialog** (공개 측 첫 소비자) — 포커스 트랩·ESC·스크림·aria 를 재구현하지 않는다. 최대 폭 480px(국내 관례, Figma 시안 없음 — 검수 시 조정), 이미지는 업로드 원본 비율 그대로(`imageWidth/Height` 로 CLS 0), 하단 바 좌 **"일주일간 보지 않기" 체크박스** / 우 **"닫기" 버튼** — 체크한 상태로 닫으면(버튼·ESC·오버레이·링크 클릭 공통) 7일 억제, 미체크 닫기는 저장 없음(새로고침 시 재노출). 상단 X 는 제거(닫기 수단 중복 — 사용자 피드백 2026-07-18, 초기 1탭 버튼안 개정). 터치 타깃 ≥44px. **이미지 클릭 링크 열기 방식은 어드민이 팝업별 선택** — `link_target` 컬럼(enum+CHECK): ① `self` 현재 탭 이동 ② `new_tab` 새 탭 ③ `small_window` 작은 새 창(`window.open` 480×≤800, 클릭 제스처 내 호출이라 팝업 차단 미적용, 보조키 클릭은 브라우저 기본) — **기본값 = small_window** (사용자 요청 2026-07-18: 사이트 이탈 없이 안내 페이지만 확인). iframe 인페이지 모달 방식은 외부 사이트 X-Frame-Options/CSP 차단으로 동작이 사이트마다 갈려 v1 제외.
...
## ADR-048: /news 히어로 캐러셀화 + 공유 카드 명칭을 헤더 메뉴명("활동 스토리")에 정합
...
운영 중 두 가지 어긋남이 사용자 제보로 드러났다.

1. **/news 히어로(FeaturedStoryCard)가 사실상 넘어가지 않는다.** 운영자가 대표글을 최대 4개 지정해도 전환 수단이 3px 막대 인디케이터뿐이라 방문자가 조작 요소로 인지하지 못한다. 또 사진·제목·설명 어디를 눌러도 반응이 없고 "자세히 보기" 버튼만 링크였다 — 카드 형태의 UI에서 기대되는 클릭 동선과 어긋난다.
2. **카톡으로 `/news` 를 공유하면 카드 제목이 "쌀 나눔 소식"** 으로 뜬다. ADR-038 에서 헤더 메뉴명을 "활동 스토리"로 정리했는데 OG 메타만 옛 명칭으로 남아, 사이트 안과 밖의 이름이 달랐다.

### Decision

1. **인디케이터 탭 → 캐러셀.** 기존 Figma 골격(좌 텍스트·우 사진 + 막대 인디케이터)은 그대로 두고 전환 수단만 추가한다 — 사진 위 좌우 화살표 + 6초 자동 넘김. 인디케이터는 현재 위치 표시 겸 직접 이동 수단으로 유지. 대표글이 1건이면 화살표·자동 넘김 모두 비활성(넘길 곳이 없음).
2. **자동 재생에는 완화 장치를 반드시 건다** — 도메인 §7 은 "과도한 애니메이션·자동 재생"을 금지하고 있어, 자동 넘김은 ① hover·focus 동안 정지(WCAG 2.2.2) ② `prefers-reduced-motion` 이면 아예 미작동 ③ 전환 효과는 0.35s opacity 페이드만(transform 없음) 세 조건 아래에서만 허용한다. 사용자가 화살표·인디케이터를 조작하면 카운트다운은 처음부터 다시 시작한다.
3. **클릭 영역 = 사진 + 제목·설명 블록.** 사진은 `absolute inset-0` 오버레이 링크로 덮되 `aria-hidden` + `tabIndex={-1}` — 제목 링크·CTA 버튼과 목적지가 같아 보조기술에 3중으로 읽히는 것을 막는다. 화살표는 오버레이보다 위(z-20)라 눌러도 글로 이동하지 않는다.
4. **공유 카드 제목은 헤더 메뉴명을 따른다** — `"활동 스토리 | 사회공헌단 Sow Good"`. 검색 키워드였던 "쌀 나눔"은 description 에 남긴다.
5. **공유 카드 이미지는 브랜드 공통 기본값(`/api/og`)을 유지한다** (삼성 뉴스룸 방식 — 페이지별로 그림은 같고 제목·설명으로 구분, 사용자 확인 2026-07-25). *히어로 대표글 커버 사진을 쓰는 안은 기각* — 운영자가 대표글을 교체할 때마다 공유 카드가 바뀌고, 카카오 캐시 때문에 한동안 옛 사진이 남아 "왜 이 사진이 뜨냐"는 문의를 만든다.

### Consequences

- Figma 125:8985 의 "인디케이터 탭 전환(캐러셀 아님)" 명세에서 의도적으로 이탈했다 — 근거는 운영 요청이며, 레이아웃·치수는 변경하지 않았다.
...
- **긴급 내리기(takedown)가 최대 7일 지연된다** (기존 4시간). ADR-004 개인정보 보호 절대 제약과 직결되므로 동의 철회 시 **R2 객체 삭제만으로 부족하고 Vercel 대시보드 purge 를 병행**해야 한다 — 운영 절차는 `docs/TODO.md` §"운영 절차 — 이미지 긴급 내리기".
- **TTL 롤백은 소급되지 않는다.** TTL 은 캐시 키가 아니라 저장 시점에 항목에 찍히므로, 값을 다시 낮춰도 이미 7일로 저장된 항목은 7일을 채운다. 완전 원복이 필요하면 purge 를 병행한다.

---

## ADR-050: 이미지 옵티마이저 원격 호스트 화이트리스트 고정 (와일드카드 제거)

- **Status**: Accepted
- **Date**: 2026-08-06
- **관련**: ADR-049 (캐시 TTL) — 같은 `next.config.ts images` 블록을 다루지만 **목적이 다르다.** 049 는 *우리 이미지의 반복 재변환* 제거, 050 은 *제3자 유입 차단*.

### Context

ADR-049 로 반복 재변환은 제거됐지만, 조사 과정에서 TTL 과 무관한 문제가 하나 더 드러났다.

`remotePatterns` 의 `*.r2.dev` 와일드카드가 **제3자 남용 경로를 열어두고 있었다.** `*.r2.dev` 는 Cloudflare 전역 네임스페이스라 누구나 무료 R2 버킷을 만들면 자기 `pub-<hash>.r2.dev` 를 갖는다. 그리고 `/_next/image` 는 **인증 없는 공개 엔드포인트**다 — 즉 제3자가 자기 이미지를 우리 변환 쿼터로 태울 수 있다.

실측(2026-07-27)으로 확인했다.

```
화이트리스트 밖 호스트  → 400   (fetch 전 차단, 과금 0)
제3자 *.r2.dev         → 502   ← 통과해서 업스트림 fetch 를 시도함
제3자 *.s3.amazonaws   → 404   ← 통과해서 업스트림 fetch 를 시도함
```

502/404 는 그 버킷이 **없어서** 난 것이고, 실존하는 제3자 객체를 가리키면 **200 + 변환 과금**이다.

나머지 둘도 문제였다 — `*.s3.amazonaws.com` 은 아예 미사용이고, `*.r2.cloudflarestorage.com` 은 코드 주석이 스스로 "인증 API 엔드포인트라 공개 렌더용 아님"이라 적어놓고 화이트리스트에 있었다.

**다만 이 리스크는 확률이 낮은 표적형이다** — 공격자가 우리 도메인을 알고 URL 을 직접 조립해야 한다. 실제로 ADR-049 의 쿼터 위기도 남용이 아니라 4시간 TTL 로 전부 설명됐다(커버 6장 × 폭 5개 × 하루 6회 ≈ 180/일). 그럼에도 고치는 이유는 **비용 비대칭** 때문이다 — 공격자는 요청 수천 건이면 한 달 쿼터를 태우고, 그러면 신규 이미지가 전부 alt 텍스트가 된다.

### Decision

**`remotePatterns` 를 실제 사용하는 호스트만의 화이트리스트로 고정한다.** 와일드카드 3종(`*.r2.dev` · `*.s3.amazonaws.com` · `*.r2.cloudflarestorage.com`)을 제거하고, prod R2 공개 버킷 1개 + 로컬 MinIO 2개만 남긴다.

```ts
remotePatterns: [
  { protocol: "https", hostname: "pub-61bc436ad01840e8b530b0e897ced05b.r2.dev" },
  { protocol: "http",  hostname: "localhost", port: "9000" },
  { protocol: "http",  hostname: "127.0.0.1", port: "9000" },
],
```

**R2 공개 도메인을 바꿀 때는 이 목록에 *추가*한다 — 기존 줄을 지우지 않는다.** 커버 URL 은 DB 에 절대 경로로 저장되므로, 옛 호스트 줄을 지우면 그 시점 이전에 발행된 글의 커버가 전부 400 이 된다.

### 기각안 — `NEXT_PUBLIC_S3_PUBLIC_URL` 파생 (초기 구현안)

env 값에서 프로토콜·호스트·포트를 파싱해 패턴을 만들고, 프로덕션 빌드에서 파생 실패 시 `throw` 하는 안을 먼저 구현했다가 **철회했다.**

- **드리프트 방지라는 이점이 실제로는 성립하지 않는다.** 커버 URL 은 DB 에 절대 경로로 저장되므로, 커스텀 도메인 전환 시 env 만 바꾸면 **기존 글 커버가 전부 400** 이 된다. "env 만 정확하면 코드 수정 불필요"는 신규 이미지에만 참이다. 와일드카드일 때는 안 깨지던 회귀를 새로 만드는 셈.
- 30줄 헬퍼 + 빌드 타임 `throw` 라는 실패 경로가 늘어난다. 얻는 것은 "호스트 문자열을 두 곳에 적지 않는다" 뿐인데, 그 문자열은 **연 단위로 바뀔까 말까 한 값**이다.
- 하드코딩 화이트리스트는 같은 보안 효과를 3줄로 낸다. 단순한 쪽을 택한다.

### 함께 기각한 것 (같은 PR 원안에 있었음)

- **`deviceSizes` 8→6 · `imageSizes` 7→1 (허용 폭 15→7)** — 분석상 대역폭 회귀는 0이었으나(3개 컴포넌트 모두 *추가된 폭 0개*), ADR-049 의 TTL 7일 적용 후에는 조합당 주 1회만 청구되어 절감이 월 수십 건에 그친다. 4-BP 렌더 재검증 비용 대비 실익이 없다. 나중에 필요해지면 그때 되살린다.
- **`formats` · `qualities` 명시** — 값이 Next 16.2.6 기본값(`["image/webp"]` · `[75]`)과 동일해 **절감 0**. 회귀 펜스로서의 가치는 있으나 설정 파일을 길게 만드는 값이 더 크다고 판단. 참고로 `qualities` 기본값이 `[75]` 단일이라 **`q` 파라미터 남용은 기본 설정만으로 이미 차단**돼 있다.

### Consequences

- **제3자가 우리 변환 쿼터를 소비하는 경로가 닫힌다.** ADR-049 가 낭비를 줄였다면 050 은 외부 유입을 막는다 — 둘은 대체재가 아니다.
- **화면·대역폭 변화 0.** 변환 파라미터를 건드리지 않았고, 우리 호스트는 그대로 통과한다.
- **R2 도메인 교체 시 코드 1줄 추가가 필요하다** (`docs/deploy-env-checklist.md` 반영). env 만 바꾸면 되는 자동화를 의도적으로 포기한 대가이며, 대신 옛 커버가 계속 산다.
- **범위 밖으로 확인된 별건** — `cacheComponents: true` 가 켜져 있는데 `"use cache"` 사용처가 0이다(`docs/tech.md` §캐싱 전략에 설계만 존재). `revalidateTag` 도 0건이라 `revalidatePath` 가 데이터 신선도에 기여하는 바가 없다. 이미지 쿼터와 무관하며 **Fluid Active CPU · Edge Requests** 를 밀어올리는 요인이다. 무효화 설계를 동반하므로 별도 스프린트로 분리한다.

---

## ADR-051: 커버 이미지 — next/image 런타임 최적화 이탈 + 업로드 시점 1440px JPEG 정규화

- **Status**: Accepted
- **Date**: 2026-08-08
- **관련**: ADR-046 (클라 리사이즈 · 원본 포맷 유지 근거) / ADR-049 (캐시 TTL) / ADR-050 (호스트 화이트리스트) — 049·050 은 *런타임 변환을 유지한 채* 낭비와 외부 유입을 막는 시도였다. 051 은 **변환 자체를 그만둔다.**

### Context

ADR-049(TTL 7일)·ADR-050(화이트리스트) 적용 후에도 Vercel 이미지 변환 Hobby 한도(5,000/월)가 소진돼, **라이브 커버 36장 중 15장(42%)이 402** 를 반환하고 `alt` 텍스트만 노출되고 있었다.

원인을 실측으로 분해했다.

| 원인 | 기여도 | 근거 |
|---|---|---|
| `minimumCacheTTL` 4h 기본값 | 약 95% | 고유 캐시 키 약 290개 × 월 17~25 사이클. 5,000 transformations / 62,000 cache write units 양쪽 검산이 일치 |
| 업로드 정규화 부재 | — | `image-resize.ts:91` 이 5MB 이하를 무처리 통과 → 평균 1,161 KB, 최대 4,133 KB, 12장이 PNG |
| 원본 초과 width srcset | — | 1162px 원본에서 `w=1200/1920/3840` 이 **97,058 B 로 바이트 동일**한데 각각 별도 과금 |
| `src` fallback = `w=3840` | — | 홈 `<img src>` 12개 전부. 크롤러·SNS 스크래퍼는 이것만 받는다 |

조사 중 **별건이 하나 더 드러났다.** 커버는 `news/[id]/page.tsx` 에서 `og:image` 로 직행하는데, 카카오톡은 `og:image` 500KB 초과 시 이미지 없는 카드로 렌더한다. **36장 중 24장(67%)이 초과** 상태였다 — 공유 썸네일이 조용히 누락되고 있었다.

### Decision

**런타임 변환을 이탈하고(`images.unoptimized: true`), 업로드 시점에 JPEG q75 / 긴 변 1440px 로 1회 정규화한다. 원본은 `original/` 에 보존한다.**

**왜 런타임 변환을 버리는가** — 값어치를 하는 3조건 중 해당하는 것이 0개다.

| 조건 | 이 프로젝트 |
|---|---|
| 원본 크기·포맷을 통제할 수 없다 | ❌ 어드민 단독 업로드 |
| 이미지가 수천~수만 장이라 사전 생성이 비현실적 | ❌ 36장, 연 증가 100장 미만 |
| 대역폭이 비용 병목이다 | ❌ Fast Data 6.2 / 100 GB |

**왜 1440px q75 인가** — 업계 기본값은 82 가 아니라 75 근방이고(imgix 75·`auto=compress` 45, Unsplash 60, Next.js 75), WordPress 의 82 는 `표시 폭 = 파일 폭` 인 고정 파생본 구조의 값이라 우리에게 맞지 않는다. Compressive Images 원리(Daan Jobsis / Filament Group)를 커버 36장에 적용해 표시 1440px 기준 RMSE 로 검증했다.

| 폭 | q | 평균 | 최대 | RMSE |
|---|---|---|---|---|
| 1200 | 82 | 138 KB | 283 KB | 9.97 |
| 1200 | 75 | 111 KB | 226 KB | **10.20** |
| **1440** | **75** | **136 KB** | **329 KB** | **5.37** |
| 1440 | 82 | 169 KB | 410 KB | 4.56 |
| 1920 | 45 | 106 KB | 330 KB | 6.30 |

1200px 은 **품질의 벽**이 있다 — q82(9.97)와 q75(10.20)의 왜곡이 사실상 같다. 1440 표시로 업스케일하는 손실이 지배적이라 품질을 올려도 개선되지 않으므로 **1200px 에 q82 를 쓰는 것은 27KB(20%)를 버리는 것**이다. 반면 1440 q75 는 1200 q82 보다 **작으면서 왜곡이 46% 적다.** 1920 이상은 표시가 1440 이라 다시 나빠진다.

1440 의 근거는 레이아웃이다 — featured 카드가 wide(1440)에서 50vw ≈ 720px CSS 로 그려져 DPR 2 기준 필요 픽셀이 정확히 1440 이다.

**왜 WebP 가 아닌 JPEG 인가** — ADR-046 의 판단(커버는 `og:image` 로 직행하고 카카오톡 스크래퍼의 WebP 지원이 보장되지 않는다)이 유효하다. 측정상 손해도 없다: 1200px JPEG(138KB)가 1600px WebP(168KB)보다 작다.

**왜 원본을 보존하는가** — Cloudinary·imgix·Shopify 는 원본 영구 보관 + 파생본 온디맨드, WordPress 는 원본 보관 + 파생본 사전 생성. 주요 플랫폼에 예외가 없다. 표시 스펙은 반드시 바뀌기 때문이다(디자인 개편·새 디바이스·AVIF·인쇄 요청). R2 저장이 무료 10GB 의 1% 수준이라 보관하지 않을 이유가 없다.

### Consequences

- **변환 쿼터 종속이 사라진다.** 402 가 발생할 경로 자체가 없어져 재발이 구조적으로 불가능하다.
- **커버 36장 41,474 KB → 4,792 KB (-88%).** 평균 1,185 → 137 KB, 최대 328 KB.
- **카카오톡 공유 썸네일 24건이 함께 정상화된다.** 500KB 초과 0건, 마진 34%.
- **모바일 전송량이 늘어난다** — 옵티마이저가 주던 66 KB 대신 137 KB. Fast Data 6.2 → 약 10 GB / 100 GB 로 여유는 충분하다. 되찾으려면 업로드 시 2벌(800/1440)을 만들어 수동 `srcset` 을 붙이면 되지만, 컴포넌트 3개가 `fill` 을 써 4-BP 재검증이 필요해 무중단 후속으로 미룬다.
- **ADR-004 긴급 내리기가 단순해진다.** 옛 구조는 R2 삭제 + Vercel purge 병행이 필요했으나 이제 **R2 삭제만으로 완결**된다.
- **EC2 이전(E3안) 후 무수정 동작한다.** 벤더 종속이 제거된다.
- **ADR-050 화이트리스트가 비활성이 된다.** `unoptimized` 에서는 `remotePatterns` 가 강제되지 않으므로, 임의 호스트 커버 URL 을 막는 것은 `news/actions.ts` 의 서버측 S3 public prefix 검증뿐이다 — 그쪽을 약화시키면 안 된다.
- **되돌릴 수 있다.** `unoptimized` 1줄 제거로 복귀하며, 정규화본은 옵티마이저 입력으로도 더 낫다(입력 1.16MB → 137KB). 백필은 새 키에 쓰고 원본을 보존하므로 DB URL 롤백만으로 원복된다.
- **브라우저 canvas 인코더는 sharp/mozjpeg 보다 18% 크다** (Chromium 실측 260 KB vs 221 KB). 목표 상한 450KB·카카오톡 500KB 대비 여유가 충분해 quality 는 조정하지 않았다.
- **범위 밖으로 확인된 별건** — 본문 인라인 이미지가 장당 1.3~1.4 MB(한 글에 6 MB)로 `<img>` raw 서빙 중이다. 커버와 달리 `og:image` 제약이 없어 규격이 다르고, 본문 HTML 안의 URL 치환이 필요해 별도 작업으로 분리한다.

---

## ADR-052: 본문 인라인 이미지 — 1810px JPEG 정규화 + 투명도 스킵 (ADR-051 후속)

- **Status**: Accepted
- **Date**: 2026-08-08
- **관련**: ADR-051 (커버 정규화 + next/image 이탈) — 같은 원인·같은 파이프라인이지만 **표시 폭과 제약이 다르다.** ADR-046 (클라 리사이즈)

### Context

ADR-051 은 커버만 다뤘다. 본문 인라인 이미지는 손대지 않았고, 실측 결과 커버보다 22배 큰 문제였다.

| | 커버 (ADR-051) | 본문 |
|---|---|---|
| 이미지 수 | 36개 | **151개** (news 143 + notices 8) |
| 총 용량 | 41.5 MB → 4.8 MB | **110.8 MB** |
| 평균 / 최대 | 137 KB / 328 KB | **763 KB / 3,442 KB** |

가장 무거운 글을 모바일 375 에서 끝까지 스크롤한 실제 네트워크 기록은 **10.4 MB** 였다(본문 9.7 MB + 관련글 커버 0.7 MB). `loading="lazy"` 는 문서 높이가 6,948px(모바일 8~9화면)이라 **끝까지 읽는 사람에게는 무효**다 — 이탈자만 절약된다.

루트 코즈는 커버와 동일하다. `image-resize.ts` 의 `file.size <= MAX_IMAGE_BYTES` 게이트가 5MB 이하를 무처리 통과시킨다. 커버는 `prepareCoverForUpload()` 로 우회했으나 본문은 여전히 `prepareImageForUpload()` 를 썼다.

**커버와 달리 장애는 아니었다.** 본문은 raw `<img>` 서빙이라 402 로 깨질 일이 없고 Fast Data Transfer 도 6.2/100 GB 로 여유가 컸다. 순수 UX 문제이며, 의도서 §6 "모바일 우선" 톤앤매너 기준에서 10 MB 는 넘어간다.

### Decision

**JPEG q75 / 긴 변 1810px + 투명도 기반 스킵.** 커버와 같은 파이프라인을 상수만 바꿔 재사용한다.

**왜 1810px** — 본문 컨테이너가 lg+ 에서 905px 이고(`news/[id]/page.tsx`), 이미지 143개 중 **105개(73%)가 `width` 속성 없이 `max-w-full` 로 컨테이너 폭까지** 늘어난다. DPR 2 기준 필요 픽셀이 905 × 2 = 1810. `width` 속성이 있는 38개도 최대 762px 이라 충분히 덮는다. ADR-051 의 커버 1440px 과 같은 논리 — 표시에 필요한 픽셀에 정확히 맞춘다.

**왜 WebP 가 아닌가** — 초기 판단은 "투명 이미지가 52% 라 알파 보존이 필수" 였으나 **오판이었다.** 알파 채널 존재 여부만 봤고 실제 투명 픽셀 비율을 재지 않았다. 전수 재측정:
...
투명 이미지는 전부 로고·아이콘류(9~26 KB)로 이미 충분히 작다. 본문은 `og:image` 로 안 나가 카카오톡 호환 제약(ADR-046)이 없어 WebP 를 쓸 수는 있지만, **이득이 전체 용량의 0.2% 범위** 안이라 커버(JPEG)와 파이프라인을 둘로 나눌 이유가 없다.

**투명도 스킵이 별도로 필요한 이유** — 크기 기반 룰로는 못 거른다. 11 KB 투명 PNG 를 JPEG 로 바꾸면 3 KB(이득 78%)라 `REENCODE_MIN_GAIN`(15%)을 통과해 버린다. 상세 본문 배경에 `bg-gradient-to-b from-white to-[#F9F4FF]/80` 라벤더 그라데이션이 깔려 있어 흰 배경 합성이 실제로 드러난다.

**되돌리기 설계 — 커버와 가장 다른 지점.** 커버는 `cover_image_url` 컬럼 하나였지만 본문은 `body`(JSONB, Tiptap 문서) **전체를 다시 쓴다.** 실패하면 글 본문이 손상되므로:

| 장치 | 내용 |
|---|---|
| body 백업 | `--apply` 시 건드릴 모든 행의 원본 `body` 를 `backup/body-backup-<ISO>.json` 에 **DB 쓰기 전** 기록 |
| `--rollback <file>` | 그 파일로 `body` 를 그대로 복원 |
| 새 키 write + `original/` 보존 | 이미지도 무손실 |

프로덕션 1개 글로 **적용 → 롤백 → 복원**까지 실측 검증했다 (217KB/136KB → 1392KB/417KB 로 정확히 복귀).

### Consequences

- **본문 이미지 110.8 MB → 15.7 MB (-86%).** 평균 908 → 134 KB.
- **가장 무거운 글 9,745 KB → 1,618 KB (6배).** LTE 20Mbps 기준 약 4초 → 0.6초.
- **재발 차단.** `prepareImageForUpload` 가 크기 무관 항상 재인코딩한다. 호출처가 `editor-image-upload.ts` 1곳이라 드롭존·"나란히 2장" 두 입력 경로가 동시에 해결된다.
- **투명 로고 17개는 원본 유지.** 흰 박스 회귀가 없다.
- **멱등성.** 재실행 시 처리 0 / 이득없음 스킵 123 / 투명 17 로 전건 스킵된다.
- **`attrs.width`/`height` 무변경.** 에디터가 정한 표시 폭(CSS px)이라 원본 픽셀과 독립이고, 축소만 하므로 렌더 결과가 바뀌지 않는다.
- **`COVER_QUALITY_LADDER` → `JPEG_QUALITY_LADDER` rename.** 커버·본문이 상한만 다르고 사다리는 공유하므로 `COVER_` 접두사가 오해를 준다.
- **범위 밖으로 확인된 별건** — 본문 이미지 151개 중 **11개가 응답·디코드 실패** 상태다. 백필 이전부터 존재하던 문제이며 스킵 처리했다. `docs/TODO.md` 에 등록.
- **`backup/` 은 gitignore.** 본문 원본이 담기므로 커밋 대상이 아니다. 검증이 끝나면 정리한다.

---

## ADR-053: 게시물 공개 노출 토글 — `is_hidden` 분리 (발행 취소와 구분)

- **Status**: Accepted
- **Date**: 2026-08-08
- **관련**: ADR-038 (랜딩 큐레이션 슬롯 — 미발행 시 슬롯 동반 해제 규칙의 예외를 만든다)

### Context

사회공헌국이 "올린 글을 잠깐 내렸다가 다시 올리고 싶다"고 요청했다. 기존 어드민 목록에는 이미 `해제` 버튼이 있었지만, 이 버튼은 요청과 다른 일을 한다.

`setPublishedAt(id, false)` 는 `published_at = null` 로 만든다 (`features/news/service.ts`). 결과:

| | 내린 직후 | 다시 올린 뒤 |
|---|---|---|
| 발행일 | 소실 (null) | **`new Date()` = 오늘** |
| 목록 순서 | 임시저장 구간으로 이동 | 원래 자리가 아닌 최상단 |
| 랜딩 슬롯 | `clearHeroRank` + `clearLandingSlots` 로 해제 | **복구 안 됨** (운영자가 수동 재지정) |
| 상태 구분 | 임시저장 | "한 번도 안 낸 글"과 뒤섞임 |

즉 3월 글을 잠깐 내렸다 올리면 8월 글이 되고, 랜딩 큐레이션 자리를 잃는다. 운영자가 기대한 "숨기기"가 아니라 **발행 취소**다.

### Decision

`news.is_hidden boolean not null default false` 를 추가해 **발행(published_at)과 노출(is_hidden)을 다른 축으로 분리**한다.

- **발행** = 최초 게시. 발행일이 확정되는 비가역 이벤트.
- **노출** = 지금 사용자에게 보이는가. 몇 번이든 왕복 가능하며 **발행일·랜딩 슬롯을 건드리지 않는다**.

공개 사이트 노출 조건은 `published_at <= now() AND is_hidden = false` 로 통일한다. 이 조건은 `features/news/db.ts` · `features/landing/db.ts` · `features/analytics/db.ts` 의 동명 헬퍼 `publicPublishedWhere()` 세 곳에 있고, **셋 다 고쳐야 한다** — 하나라도 빠지면 숨긴 글이 그 표면으로 샌다.

숨김은 *이미 공개 시각이 지난 글*에만 의미를 둔다. 임시저장·예약은 아직 사용자에게 보인 적이 없어 `is_hidden` 을 무시한다 (`features/news/publish-state.ts`) — 예약글에 "비공개" 배지를 붙이면 "예약이 취소된 건가?"로 읽힌다.

### UI

4개 안(인라인 스위치 / 상태 배지 드롭다운 / 눈 아이콘 + 행 흐림 / 벌크 + 확인창)을 실제 클릭 가능한 비교 보드로 만들어 사용자가 선택했다. 채택안은 **눈 아이콘 + 행 흐림**:

- 목록에 `노출` 열 신설. 공개/비공개 글에만 눈 아이콘, 예약글은 `발행 대기` 텍스트.
- 비공개 행은 `opacity-55` 로 흐려 목록을 훑는 것만으로 눈에 띈다 — 배지 색 단독 의존을 피한다(접근성).
- 상태 탭에 `비공개` 추가. 기존 `발행` 라벨은 `공개` 로 바꿔 노출 축과 어휘를 맞췄다.
- 터치타겟은 모바일 44px / 데스크탑 36px (`size-11 md:size-9`).

**공개된 글의 `해제` 버튼은 목록에서 제거했다.** 파괴적 조작을 한 클릭 거리에 두지 않는다. 임시저장으로 되돌리는 경로는 에디터의 `임시 저장` 버튼으로 남아 있다. 임시저장 글의 `발행`, 예약글의 `예약 취소` 버튼은 유지된다.

### Consequences

- **ADR-038 의 "미발행 시 슬롯 동반 해제" 는 발행 취소에만 적용된다.** 노출 토글은 슬롯을 유지하고, 숨긴 동안에는 `publicPublishedWhere()` 가 랜딩 쿼리에서 걸러낸다. 슬롯은 점유된 채 비어 보이므로, 장기간 숨길 글은 운영자가 슬롯을 직접 비우는 편이 낫다.
- **마이그레이션 0016 은 additive** — 기존 15건 모두 `is_hidden = false` 로 공개 상태가 유지된다.
- 대시보드 글 현황에 `hidden` 카운트가 추가되고, 기존 `published` 카운트는 숨긴 글을 제외한다.
- 공개 표면 검증은 4곳(상세·`/news` 목록·랜딩·sitemap) 왕복으로 확인했다. 새 공개 쿼리를 추가할 때 `publicPublishedWhere()` 를 거치지 않으면 이 보장이 깨진다.

---

## ADR-054: 랜딩 메인 스토리 — 지정 자리 12칸 + 노출 개수 운영자 설정 (ADR-038 개정)

- **Status**: Accepted
- **Date**: 2026-08-13
- **관련**: ADR-038 (랜딩 큐레이션 슬롯 — 하단 슬롯 수를 7 로 고정했던 결정을 개정)

### Context

사회공헌국이 "메인 스토리 목록을 10개까지 고를 수 있게 해달라"고 요청하면서 "노출 개수는 원래 설정할 수 있었던 것 같다"고 덧붙였다. 코드를 확인한 결과 **그런 설정은 없었고, 지정 자리와 노출 개수가 서로 어긋난 채 굳어 있었다.**

| | 값 | 위치 |
|---|---|---|
| 어드민이 지정 가능한 자리 | 7 | `LandingSlotManager.tsx` 로컬 상수 |
| 공개 랜딩에 실제 노출 | 6 | `app/(public)/page.tsx` 의 `.slice(0, 6)` |

7번 자리는 어드민에서 고를 수 있고 미리보기에도 뜨지만 **사용자 화면에는 절대 나오지 않는 죽은 슬롯**이었다. 운영자 입장에서는 "지정했는데 안 보인다"로 드러난다.

원인은 값이 상수 하나가 아니라 **`7` 이 7곳, `6` 이 3곳에 각각 하드코딩**돼 있었다는 것이다. 공유 상수가 없어 한쪽만 고치면 조용히 어긋난다.

### Decision

**지정 자리와 노출 개수를 다른 축으로 분리한다.**

- **지정 자리** = `FEATURED_SLOT_MAX = 12` 고정. 운영자가 미리 골라둘 수 있는 최대치.
- **노출 개수** = 운영자가 1~12 중 선택. 랜딩에 실제로 렌더되는 카드 수.

값은 `src/features/landing/constants/slots.ts` 단일 출처로 모으고, 흩어져 있던 10곳의 하드코딩을 전부 제거했다.

노출 개수는 신규 `site_settings` 테이블에 저장한다. **단일 행(`CHECK id = 1`) + 명시 컬럼** 방식 — key-value 가 아니다. 값이 정수 하나라 `text` 저장은 매 읽기마다 파싱·검증을 요구하고, 명시 컬럼이면 drizzle-zod 브릿지와 타입이 그대로 산다. 설정이 늘면 컬럼을 추가한다 (ADR-039 Expand/Contract 가산형).

**행은 마이그레이션이 만들지 않는다.** 읽기는 행이 없으면 기본값 6 을 반환하고, 저장 시 `onConflictDoUpdate` 로 upsert 한다. 마이그레이션에 데이터 조작을 넣지 않는다는 ADR-039 원칙을 지키면서 최초 배포 직후에도 랜딩이 정상 동작한다.

### 어드민 UI

노출 개수를 넘는 자리는 **`대기` 배지 + 점선 테두리**로 표시하고, 노출 경계에 `여기까지 화면에 노출` 구분선을 넣는다. 7번 슬롯이 조용히 사라지던 문제의 재발 방지 — 지정은 되지만 지금은 안 보인다는 사실이 화면에 드러나야 한다.

### 동반 수정 (같은 코드 경로에 걸려 있던 버그 2건)

1. **`revalidatePath("/admin/main-story")` 누락** — featured 슬롯을 실제로 편집하는 페이지가 무효화 대상에서 빠져 있었다 (`/admin/landing` 만 있었다). `revalidateNewsRoutes` 와 `setLandingSlotAction` 양쪽에 추가.

2. **노출 범위 밖 pin 글이 사라지던 버그** — `listFeaturedGrid` 는 `pinnedIds` 에 범위 밖 rank 까지 넣어 fallback 후보에서 제외하면서, `filledSlots` 에는 안 넣었다. 노출 6인데 9번에 지정된 글은 **랜딩 어디에도 나오지 않았다.** 노출 개수를 줄이면 바로 이 경로를 타므로 이번에 함께 고쳤다: 범위 밖 rank 는 '미지정'과 동일 취급해 최신순 fallback 후보로 되돌린다. fallback where 의 `isNull(featured_rank)` 조건을 제거하고 "이미 자리를 차지한 글"만 제외하는 방식으로 바꿨다.

### Consequences

- 마이그레이션 `0018` 은 순수 `CREATE TABLE` — 기존 데이터 무영향, 롤백은 테이블 DROP.
- 랜딩 하단이 최대 12칸까지 길어질 수 있다. 마조네리는 이미 가변 개수를 처리하므로 레이아웃 변경은 없지만, 개수를 크게 잡으면 스크롤이 길어진다 — 운영 판단에 맡긴다.
- `db/seed.ts` 의 `featuredRank?: 1|2|…|6` 타입은 **의도적으로 넓히지 않았다.** 시드 샘플이 6건뿐이고, 좁은 리터럴 유니온이 정수 중복(unique index 23505)을 리뷰 시점에 드러내는 장치이기 때문이다 (seed.ts:123 주석).
- 새 노출 관련 코드는 반드시 `slots.ts` 상수를 거쳐야 한다. 숫자를 다시 인라인하면 이 ADR 이 고친 문제가 그대로 재발한다.

---

## ADR-055: 홈 팝업 — "보지 않기" 기간을 팝업별 선택으로 (ADR-047 §4 개정)

- **Status**: Accepted
- **Date**: 2026-08-13
- **관련**: ADR-047 (홈 팝업 v1 — localStorage 7일 억제를 고정값으로 뒀던 결정을 개정)

### Context

사회공헌국이 "팝업별로 하루 보지 않기 / 일주일 보지 않기를 고를 수 있냐, 실제로 동작하는지도 알려달라"고 물었다.

행사 모집처럼 마감이 임박한 안내는 매일 다시 보이는 편이 낫고, 상시 안내는 일주일이 적당하다. 지금은 `WEEK_MS = 7 * 24 * 60 * 60 * 1000` 이 `popup-dismiss.ts` 에 고정돼 있어 팝업 성격과 무관하게 7일이다.

### Decision

`popups.dismiss_duration text not null default 'week'` (`'day' | 'week'`) 을 추가해 팝업별로 선택한다. 어드민 폼에 Select 1개가 늘고, 공개 체크박스는 **개수를 늘리지 않고 문구만 바뀐다** — `하루 동안 보지 않기` / `일주일간 보지 않기`.

**동작 가능 여부: 가능하다.** 기존 저장 구조가 이미 팝업 ID별 만료 시각 맵(`{ [popupId]: 만료 epoch ms }`)이라, 고정 상수를 팝업별 값으로 바꾸는 것 외에 구조 변경이 없다. 기존에 저장된 값도 만료 epoch 이라 그대로 호환되어 마이그레이션·초기화가 필요 없다.

**"하루"는 자정 기준이 아니라 닫은 시점부터 24시간이다.** 주(7×24h)와 계산식이 같아 맵 하나로 끝나고, 밤 11시에 닫은 사람이 1시간 뒤 다시 보게 되는 상황을 피한다. 라벨도 "오늘 하루"가 아니라 "하루 동안"으로 맞췄다.

### 한계 (ADR-047 §4 에서 그대로 승계)

브라우저·기기별로만 동작한다. 시크릿 모드, 다른 브라우저, 캐시 삭제 시 다시 뜬다. "1인 1회 보장"이 아니라 **"같은 브라우저에서 재노출 완화"** 다. 서버 저장으로 바꾸려면 방문자 식별이 필요해져 개인정보 절대 제약(ADR-004)과 충돌하므로, 이 한계는 유지한다. 어드민 도움말에 이 사실을 명시했다.

### Consequences

- 마이그레이션 `0017` 은 가산형 컬럼 1개 + CHECK 제약. 기존 팝업은 전부 `week` 로 유지되어 동작 변화가 없다.
- `dismissPopupForWeek(id)` → `dismissPopup(id, duration)` 로 시그니처가 바뀌었다. localStorage 키(`sg_popup_dismissed`)와 저장 형태는 그대로다.
- 기간을 3종 이상으로 늘리려면 `DISMISS_DURATIONS` 배열과 CHECK 제약, 공개 라벨 맵 3곳을 함께 고쳐야 한다.

---

## ADR-056: 언론 속 사회공헌 — `board` 구분 컬럼 + 리포지토리 스코프 (ADR-042 패턴 미적용)

- **Status**: Accepted
- **Date**: 2026-08-13
- **관련**: ADR-042 (공지사항 별도 도메인 — 이번에는 같은 결론을 내리지 **않은** 이유를 아래에 기록), ADR-025 (카테고리 동적 관리), ADR-038 (랜딩 큐레이션 슬롯)

### Context

사회공헌국이 "언론 속 사회공헌" 메뉴와 화면을 요청했다. 확정된 요구:

- 활동 스토리와 **글·카테고리가 분리된 별도 게시판**
- 어드민에는 **카테고리 관리 + 스토리 관리 2개만** (강조 캐러셀 관리 없음)
- 언론사명·원문 링크 필드는 **넣지 않음**
- 공감·검색·정렬·조회수는 `/news` 와 **전부 동일하게**

### Decision

`news`·`categories` 에 `board text not null default 'story'` (`'story' | 'press'`) 를 추가한다. 테이블은 나누지 않는다.

**판단 기준은 코드량이 아니라 "두 엔티티가 같은가"였다.** 언론사 필드를 넣지 않기로 하고 공감·조회수까지 동일하게 가져가기로 확정된 순간, 언론 보도는 활동 스토리와 **필드 구성이 100% 같은 엔티티**가 됐다. 같은 것을 두 테이블로 나누는 건 모델링이 아니라 애플리케이션 레벨 파티셔닝이고, 대가는 영구적인 코드 이중화다.

**ADR-042(공지사항 분리)를 따르지 않은 이유**: 공지사항은 실제로 달랐다 — 첨부파일이 있고, 카테고리가 없고, 상위 고정이 있고, 공감이 없다. 언론 보도는 그 중 어느 것도 해당하지 않는다. 선례는 상황이 같을 때만 선례다.

### 필터 누락을 구조로 막는다

`board` 방식의 유일한 실질 위험은 "쿼리에 조건을 빠뜨리면 컴파일 에러 없이 데이터가 샌다"이다. 이걸 세 층에서 막는다.

**1층 — 기본값 없는 필수 인자.** `news/db.ts` 와 `categories/db.ts` 의 조회 함수는 `board` 를 **첫 인자로 강제**한다. 기본값을 주지 않는 게 핵심이다. 도입 시점에 `tsc` 가 누락 호출부 **17곳을 전부** 컴파일 에러로 드러냈고, 앞으로 추가되는 쿼리도 같은 방식으로 잡힌다.

**2층 — DB CHECK 제약.** 랜딩·소식 큐레이션 슬롯은 활동 스토리 전용이므로 DB가 직접 막는다.

```sql
CHECK (board = 'story' OR (story_slot IS NULL AND featured_rank IS NULL AND hero_rank IS NULL))
```

애플리케이션에 버그가 있어도 언론 글은 슬롯을 점유할 수 없다. 덕분에 기존 partial unique 3개(`news_story_slot_uniq` 등)에 board 술어를 넣어 재작성할 필요가 없었다.

**3층 — 진입점 화이트리스트.** 공개 read 채널 `/api/news` 는 `z.enum(NEWS_BOARDS).default("story")` 로 board 를 검증한다. 임의 문자열이 DAL 까지 흘러가지 않는다.

### 카테고리 slug — 복합 unique

`categories.slug` 는 전역 unique 였다. 두 게시판이 "쌀 나눔" 같은 같은 이름을 각자 쓰려면 이걸 `(board, slug)` 복합으로 바꿔야 한다.

바꾼 뒤 **반드시 함께 고쳐야 하는 곳이 있다**: `getRiceSharingCategoryId()` 는 `WHERE slug = 'rice_sharing'` 단독 조회에 `limit(1)` 이라, 두 행이 존재하는 순간 어느 쪽이 올지 정해지지 않는다. 이 함수는 랜딩 "밥이 사랑이다" 슬롯의 쌀 나눔 판정에 쓰이므로, 잘못 집으면 **에러 없이 조용히** 판정이 틀어진다. `AND board = 'story'` 를 추가했다.

### 게시판 간 경계

| 기능 | 처리 |
|---|---|
| 랜딩 슬롯 3종(story/featured/hero) | 활동 스토리 전용 — 쿼리 필터 + DB CHECK |
| 목록·상세·카테고리 탭 | 게시판별 완전 분리 |
| 관련 글·이전/다음 | 같은 게시판 안에서만 (보충 fallback 포함) |
| 태그 자동완성 | `news` 조인으로 게시판별 태그 풀만 제안 |
| 공감·조회수·분석 | **공유** (사용자 결정) — `news_id` FK 그대로 재사용 |
| RQ 캐시 키 | `board` 를 키 앞자리에 포함 — 빠지면 두 목록이 서로의 캐시를 보여준다 |
| revalidatePath | 게시판별 경로만. 언론 글 수정이 랜딩을 다시 그리지 않는다 |

**분석 집계에서 발견한 실제 버그**: 어드민 대시보드 인기글은 board 무관하게 집계하는데 링크가 `/admin/news/{id}/edit` 하드코딩이었다. 언론 글이 인기글에 오르면 404 로 간다. `topNews` 가 `board` 를 함께 실어 각 게시판 편집 화면으로 보내도록 고쳤다. 컴파일러가 못 잡은 유일한 사례이고, board 방식에서 이런 지점은 "집계는 공유하되 링크는 게시판별"인 곳에 남는다.

### 공개 헤더 — 메뉴 5개

메뉴가 4 → 5개가 되면서 `SectionContainer` 밴드 폭 기준으로 768~1023(648px) 구간에 안 들어간다. 풀내비 진입 BP 를 `md:`(768) → `lg:`(1024) 로 올리고, lg 구간은 간격·타이포를 줄여 담았다(`gap-2`/`px-3`/`text-sm`). wide(1440)에서 Figma 원형(`gap-6`/`px-5`/`text-base`)을 복원한다. 768~1023 은 이미 완성돼 있던 모바일 드롭다운을 그대로 쓴다.

새 메뉴에는 `section` 을 주지 않았다 — `SCROLL_SECTIONS` 에 섞이면 랜딩 스크롤스파이가 깨진다.

### 분리 트리거 (다음 사람을 위한 판단 기준)

"나중에 언론 게시글 구성이 바뀔 수 있다"는 막연한 가능성만으로는 분리하지 않는다. 아래 중 하나라도 참이면 그때 검토한다.

1. 언론 전용 nullable 컬럼이 **4개**를 넘음 → 먼저 `press_meta` 1:1 사이드카 테이블로
2. 서비스 레이어에 `if (board === "press")` 분기가 **3곳** 이상 생김
3. `body` 또는 `published_at` 의 **의미**가 board 마다 달라짐 → 유예 없이 즉시 분리
4. 언론 보도만 외부 API 자동 수집 대상이 됨 → 즉시 분리

3번이 한계선이다. 같은 컬럼이 두 의미를 갖는 순간 단일 테이블의 근거가 사라진다.

**되돌리는 비용이 비대칭이라는 점이 이 결정을 안전하게 만든다.** 데이터가 이미 board 로 나뉘어 있어 `CREATE TABLE press_articles AS SELECT ... WHERE board='press'` 로 기계적으로 분리할 수 있고, 호출부는 리포지토리 인터페이스 뒤에 있어 거의 바뀌지 않는다. 반대 방향(나눴다가 합치기)은 그동안 갈라진 두 벌의 코드와 스키마를 조율해야 해서 훨씬 비싸다.

### Consequences

- 마이그레이션 `0019` — 컬럼 2개 + CHECK 3개 + 인덱스 2개 추가, `categories_slug_unique` 를 복합으로 교체. `DEFAULT 'story'` 라 **기존 글·카테고리는 전부 활동 스토리로 자동 귀속**되고 화면 변화가 없다.
...
## ADR-061: 에디터 글꼴 목록 재구성 — 인기·용량 실측 기반 7종

- **Status**: Accepted (ADR-059 의 목록 부분 개정)
- **Date**: 2026-08-28

### Context

ADR-059 로 글꼴 선택을 넣은 직후 사용자가 물었다 — *"워드·한글에서 자주 쓰는, 웹에서 많이 쓰는
폰트들을 좀 자세히 확인해서 추천해달라. 구글에서 추천하는 폰트도 봐달라."*

구글 폰트 전체 메타데이터(`fonts.google.com/metadata/fonts`, 1,946 패밀리)를 받아 분석했다.

**핵심 발견 ① — 전세계 인기 상위는 전부 한글 미지원.**
...
한국어(`korean` subset) 지원은 **1,946 종 중 38 종**뿐이다. 라틴 전용 글꼴을 드롭다운에 넣으면
운영자가 고른 뒤에도 한글 본문이 사이트 기본 글꼴로 폴백해 **"아무 일도 안 일어난"** 것처럼 보인다.
이 사이트는 한국어 전용(ADR-011)이라 값어치가 없다 → 후보를 38 종으로 한정했다.

**핵심 발견 ② — 워드·한글 기본 폰트 중 굴림·돋움은 OFL 로 쓸 수 있다.**

맑은 고딕(MS)·함초롬(한컴)은 독점이라 불가. 반면 굴림·돋움·바탕·궁서는 원 제작사
한양정보통신(HanYang I&C) 명의로 2024-05 구글 폰트에 온보딩됐다. `ofl/gulim/upstream_info.md` 원문:

> Gulim is a Korean gothic-style font originally designed by HanYang I&C Co. and first shipped with
> Windows 95. It was onboarded to Google Fonts in May 2024 by Aaron Bell.

용량도 압도적으로 가볍다 — **한글 전역 11,172 자가 단일 파일 122 KB**(분할조차 필요 없음).

**핵심 발견 ③ — 인기순으로 보면 기존 선정이 최적이 아니었다.**

| 순위 | 패밀리 | 웹 전송량 | 청크당 | ADR-059 |
|---|---|---|---|---|
| 36 | Noto Sans KR | 6.9 MB | 27 KB | 미포함 |
| 100 | 나눔고딕 | 2.0 MB | 11 KB | 포함 |
| 187 | 나눔명조 | 2.6 MB | 14 KB | 포함 |
| 205 | 본명조 | 12.2 MB | **49 KB** | 포함 |
| **246** | **Gothic A1** | **1.6 MB** | **7 KB** | 미포함 |
| 541 | 고운바탕 | 3.0 MB | 16 KB | 포함 |
| 818 | 고운돋음 | 1.3 MB | 13 KB | 미포함 |
| 918 | 개구 | 1.3 MB | 6 KB | 포함 |

### Decision

**7 종**: 기본(SUIT) · 나눔고딕 · 고딕 A1 · 고운돋음 · 나눔명조 · 고운바탕 · 개구.
드롭다운은 고딕 → 명조 → 손글씨 순으로 묶는다.

1. **고딕 A1 추가.** 인기 246 위인데 청크당 7 KB 로 분할형 중 가장 가볍다.
2. **고운돋음 추가.** 고운바탕과 짝이라 명조·고딕을 같은 톤으로 섞을 수 있다. 400 단일 웨이트라
   굵게는 브라우저 합성(faux bold)이다 — 700 을 요청해도 구글이 400 만 준다.
3. **본명조 제거.** 청크당 49 KB 로 나머지의 4~6 배. 명조 수요는 나눔명조·고운바탕이 덮는다.
4. **굴림·돋움 제외** — 사용자 결정. OFL 이고 CSS2 API 로 서빙되지만 `fonts.google.com`
   **공개 카탈로그에 미등재**다(메타데이터 목록에 없음). 카탈로그에서 빠진 상태로 CDN 에 의존하는
   불확실성을 안고 갈 이유가 없다고 판단했다.
5. **라틴 전용 글꼴 미포함** — 발견 ① 의 이유. 넣으면 고장난 항목이 된다.
6. **Pretendard 보류** — SIL OFL 1.1 이고 국내 웹 표준급이지만 구글 폰트 미수록이라
   jsDelivr·자체 호스팅 경로를 하나 더 만들어야 한다. `docs/TODO.md` 로.

### Consequences

- **본명조로 이미 발행된 글은 사이트 기본 글꼴(SUIT, 고딕)로 렌더된다.** serif 로 떨어지지 않는다 —
  `normalizeFontFamily` 가 null 을 주면 sanitize 가 textStyle 마크를 통째로 drop 하기 때문이다.
  ADR-059 배포 15 분 뒤의 제거라 실제 사용 글은 없다. 되살리려면 `EDITOR_FONTS` 에 다시 넣기만 하면
  옛 글도 함께 복구된다 — 저장값이 패밀리명 하나라서 가능한 일이다.
  `font-family.test.ts` 가 이 동작을 명시적으로 고정한다.
- 굴림·돋움이 필요해지면 자체 호스팅이 현실적이다 — 122 KB / 128 KB 로 두 개 합쳐 250 KB 다.
- 목록 근거를 `editor-allowlist.ts` 주석에 남겼다. 다음 사람이 "왜 Roboto 가 없나"를 다시 조사하지 않도록.
