...
- **SEO 하드닝 — branch `feat/seo-hardening`** — `docs/plans/active/TASK-20260715-seo-hardening.md` (ADR-044). JSON-LD 3종 + sitemap notices 편입 + GSC 소유확인(HTML 파일) + admin X-Robots-Tag + sr-only h1 + RSS `/feed.xml`. tsc0·lint0·test108·build✓·로컬 prod 런타임 8종 통과. **스키마 변경 0.**
  - 🔴 **배포 후 1회 운영 절차** (TASK 파일 §배포 후 운영 절차): GSC 소유확인 '확인' 클릭 → sitemap 제출 → 주요 URL 색인 요청 → GA4 연결. 네이버 서치어드바이저 등록 권장(RSS `/feed.xml` 제출).
  - GSC 속성 `https://sowgood.kr/`(URL 접두어)은 2026-07-15 생성 완료·**인증 대기** 상태. `public/google7f8c005c70787329.html` 삭제 금지.
