...
> 2026-07-27 부터 이미지 캐시 TTL 이 4시간 → **31일** 이다 (Vercel 변환 쿼터 82% 도달 대응). 그 대가로 **R2 객체를 지워도 Vercel CDN 이 최대 31일 계속 서빙한다.** ADR-004 개인정보 보호 절대 제약과 직결되므로 아래 3단계를 반드시 지킨다.

1. 어드민에서 해당 글 **비발행** (목록·상세에서 사라짐)
2. Cloudflare R2 콘솔에서 **해당 객체 삭제** (원본 직접 URL 차단)
3. Vercel 대시보드 → 해당 소스 이미지 **purge** ← **이걸 빼면 최대 31일 계속 서빙된다**
