...
### 🔴 쌀나눔 시트 공유 범위 — 실명 노출 (2026-08-28, ADR-004 / ADR-058)

- 시트: `docs.google.com/spreadsheets/d/1rHTzNuqFA1d0BQFhmThIGEwJZsK5hFHPJttbhqzV1vw`
- **현재 '링크가 있는 모든 사용자' 공유** — 링크만 알면 누구나 열람 가능. 인증 없이 CSV export 가 열리는 것으로 확인(2026-08-28).
- 시트에 담긴 개인정보: 실명(`오인철 서울남부 부교구장 부친 및 오충완 경기북부 교구장 조부 성화 쌀화환`), 수혜 기관명(흑석종합사회복지관·삼태기마을·철원/용산 푸드뱅크).
- 사이트로 나가는 값은 총계 숫자 3개(3,210 / 106 / 6)뿐이라 **웹사이트를 통한 노출 경로는 없다**. 문제는 시트 자체의 공유 범위다.
- **요청:** 공유를 '제한됨' 으로 되돌릴지 결정. 되돌리면 협회 KPI 시트와 같은 Apps Script 웹앱 경유가 필요하고, 우리 쪽은 `RICE_SHEET_CSV_URL` 환경변수만 교체하면 된다(코드 변경 0).

### 쌀나눔 통계 4번째 항목 (2026-08-28, 보류)

- 시트에 `쌀화환 참여기관 수`(175)·`쌀 나눔 포대 수`(323) 가 더 있으나 랜딩에는 3개만 반영(사용자 결정 — Figma 3열 유지 + '참여기관'은 기부처라 의미 축이 다름).
- 사회공헌국이 4번째 항목을 원하면 StorySection 통계 블록의 4-BP 레이아웃 재검증이 필요(+1~2h).
...
- [ ] **dev 에서 `notFound()` 가 200 을 반환** — 존재하지 않는 UUID 로도 재현되는 기존 동작(PPR 스트리밍이 헤더 선전송). 404 본문은 정상 렌더된다. prod 빌드에서 상태 코드가 404 로 나가는지 확인 필요 — SEO 영향.
- [ ] **에디터 hydration 경고** — `DateTimePicker` 의 `오전 9:00` vs `AM 9:00` 로케일 불일치. 공개 글 에디터에서도 동일 재현되는 기존 문제. 서버·클라 동일 포맷터로 고정 필요.
- [ ] **장기 비공개 글의 랜딩 슬롯 (사회공헌국)** — 숨긴 글이 `story_slot`/`featured_rank` 를 계속 점유한다(되돌릴 때 자리 보존이 목적). 오래 숨길 글은 슬롯을 비우는 편이 낫다는 안내가 필요한지 판단.
