...
## 운영 절차 — 이미지 긴급 내리기 (ADR-049 필수 동반)

> 2026-08-06 부터 이미지 캐시 TTL 이 4시간 → **7일** 이다 (ADR-049, Vercel 변환 한도 97% 도달 대응). 그 대가로 **R2 객체를 지워도 Vercel CDN 이 최대 7일 계속 서빙한다.** ADR-004 개인정보 보호 절대 제약과 직결되므로 아래 3단계를 반드시 지킨다.

1. 어드민에서 해당 글 **비발행** (목록·상세에서 사라짐)
2. Cloudflare R2 콘솔에서 **해당 객체 삭제** (원본 직접 URL 차단)
3. Vercel 대시보드 → 해당 소스 이미지 **purge** ← **이걸 빼면 최대 7일 계속 서빙된다**

⚠️ 브라우저가 이미 받아간 사용자는 purge 로도 회수되지 않는다. 3단계는 "그 이후 새로 접근하는 사람"을 막는 조치다.
...
### 이미지 최적화 후속 (2026-08-06, ADR-049·050)
...
- [ ] **ADR-050 배포 후 curl 검증 3종** — 서버 측 강제는 배포 후에만 확인 가능하다.
      ① 제3자 호스트 차단: `pub-<임의>.r2.dev` · `no-such-bucket.s3.amazonaws.com` → **둘 다 400** (배포 전에는 502/404 로 통과했음)
      ② 제거한 폭 거부: `w=256` · `w=2048` · `w=3840` → **전부 400**
      ③ 커버 정상 렌더 육안 확인 (랜딩 · `/news` 목록 · `/news/[id]` · `/notices`)
