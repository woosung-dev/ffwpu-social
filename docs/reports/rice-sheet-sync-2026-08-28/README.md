...
## S14. 천단위 콤마와 단위가 붙는가 (사용자 확인 요청)

리포트 S11 에 붙인 `8127` · `16078` 은 **DB 컬럼 원값**이다. 화면 표시는 `formatKpiDisplay` 가
`value.toLocaleString("ko-KR") + (unit ?? "")` 로 조립한다.

```
8127  + 명    →  8,127명
16078 + 시간  →  16,078시간
16078 + null  →  16,078          ← 단위가 비면 맨숫자
80257 + 개    →  80,257개
614   + 회    →  614회            ← 1000 미만은 콤마 없음(정상)
```

**콤마는 이미 붙는다.** 문제는 단위였고, seed 에 실제 구멍이 있었다:

`volunteer_period` 는 원래 비숫자 표기 카드("38년 5개월")여서 `value: null, unit: null` 로 시드됐다.
그런데 ADR-058 매핑에서 시트 `연인원봉사시간 누계` 가 **이 slug 에 시간 숫자를 넣기 시작했다**.
`unit` 이 비어 있으면 첫 동기화 직후 랜딩에 `16,078` 이 단위 없이 노출된다.

**수정**: seed 의 `unit: null` → `"시간"` + 마이그레이션 0022 로 기존 행 backfill
(`unit IS NULL OR unit=''` 가드 — 운영자 입력 우선, 0020 과 동일 패턴).

```
-- 단위를 비운 상태에서 0022 적용
       slug       | value | unit
------------------+-------+------
 volunteer_period | 16078 |          ← 적용 전

          slug          | value | unit
------------------------+-------+------
 volunteer_count        |  8127 | 명
 volunteer_period       | 16078 | 시간   ← 채워짐
 helped_household_count | 80257 | 개
 event_count            |   614 | 회
```

**렌더 결과 PASS**

![kpi section](08-kpi-section-comma-unit.png)

`8,127명` · `16,078시간` · `80,257개` · `614회` — 네 장 모두 콤마 + 단위.

> 단위는 `kpi_metrics.unit` 컬럼이 원본이고 운영자가 `/admin/kpi` 의 **단위 칸**에서 소유한다.
...
2. 마이그레이션 `0020`·`0021`·`0022` 적용 (migrate 워크플로)
