...
- 완료(미머지): **소식 검색 + 768 그리드 정정** (branch `feat/news-search`) — `/news` "더 많은 소식" 탭+검색 인라인 툴바(제목+태그 ILIKE, q×category AND), 768 카드 3열→2열 수정. Generator-Evaluator 워크플로우(baseline 4BP → 구현 TDD → 2-pass 적대검증 → codex C1 반복q 500 수락 → docs). ADR-036. 디테일 `docs/plans/active/2026-06-07-news-search.md` · 검증 `docs/design/review-news-search-2026-06-07.md`. tsc0·lint0·test47. **스키마 변경 0.**
