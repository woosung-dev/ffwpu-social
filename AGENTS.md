...
- 진행 중: **어드민 이미지 업로드 UX** (branch `feat/admin-image-upload-ux`) — 디테일 `docs/plans/active/TASK-20260716-admin-image-upload-ux.md`. 에디터 이미지 업로드가 **콘솔에만 에러 찍고 침묵**하던 문제(`File size exceeds maximum allowed (5MB)`). ① `onError`→한국어 토스트(벤더 영문은 경계에서 번역) ② 업로드 전 자동 리사이즈 — 긴 변 2560·webp 품질 사다리, 이미 작으면 무손실 통과. 삽입 지점=`makeBodyImageUploader`(드롭존+2장나란히 **공통 길목**)+`CoverImageUploader`. **저장 상한 5MB 유지 · 원본 상한 30MB 신설**(벤더 게이트가 리사이즈보다 먼저 도는 탓). 5MB 중복선언 → 순수 모듈 `features/storage/image-policy.ts` SSOT(`attachment-policy` 선례, 배럴은 server-only). ADR-046. **스키마 0**. tsc0·lint0·test115 · 실측 13.91MB→**2.79MB webp**(3경로)·작은건 SHA동일·공개렌더✓. **커밋 승인 대기.**
