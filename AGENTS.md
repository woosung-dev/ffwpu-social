...
- 완료(미머지): **SEO 하드닝** (branch `feat/seo-hardening`) — JSON-LD 3종(랜딩 Organization+WebSite·소식 NewsArticle·공지 Article) + sitemap notices 편입·lastmod 정확화 + GSC 소유확인(HTML 파일 primary·env 예비) + admin 호스트 X-Robots-Tag + /news sr-only h1 + RSS `/feed.xml`(네이버 제출용). ADR-**044**. tsc0·lint0·**test108**·build✓·로컬 prod 런타임 8종 통과. **스키마 변경 0.** 🔴 배포 후 GSC 소유확인 '확인' 클릭→sitemap 제출→색인 요청 (TASK §운영 절차). 실측: 구글 색인 17 URL·브랜드 검색 1~3위·PSI SEO **100**(모바일 성능 70·데스크톱 88). 디테일 `docs/plans/active/TASK-20260715-seo-hardening.md`.
